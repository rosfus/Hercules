	...
	case C_RE_NE:
		{
			...
			compiled_regex = pcre_compile(s2, 0, &pcre_error, &pcre_erroroffset, NULL);
			...
			extra_regex = pcre_study(compiled_regex, 0, &pcre_error);
			...
			offsetcount = pcre_exec(compiled_regex, extra_regex, s1, inputlen, 0, 0, offsets, 256*3);
			...
			if( op == C_RE_EQ ) {
				for( i = 0; i < offsetcount; i++ ) {
					pcre_get_substring(s1, offsets, offsetcount, i, &pcre_match);
					mapreg->setregstr(reference_uid(script->add_str("$@regexmatch$"), i), pcre_match);
					pcre_free_substring(pcre_match);
				}
